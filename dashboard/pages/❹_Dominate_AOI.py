...
for i in range(len(step_index)-1):
    ...
    dominate_aoi_impulse.append(find_dominate_aoi(fixations=fixations[step_index[i]:step_index[i+1]]))
...
df_impulse = pd.DataFrame(dominate_aoi_impulse, columns=('Time %d' % i for i in range(1, len(step_index))))
